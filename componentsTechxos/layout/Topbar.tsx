...
const Topbar = () => {
  ...
  // Admin route that will be conditionally rendered
  const adminRoute: Route = { label: "ADMIN", path: "/admin/users" };
  
  const createCourseRoute: Route = { label: "CREATE", path: "/instructor/courses" };
        ...
        <nav className="flex w-full mx-auto justify-between bg-white h-[80px] items-center p-4 border-b-2 border-[#E79D09]">
          ...
          <div className="flex items-center font-semibold gap-4">
            ...
            <div className="hidden lg:flex md:text-[13px] gap-2">
              {topRoutes.map((route: Route) =>
                route.label === "OUR COURSES" ? (
                  ...
                  >
                    ...
                    {/* Dropdown Content */}
                    <div
                      className={`fixed top-[120px] right-0 w-[65vw] bg-white font-semibold text-[#003B65] shadow-lg flex flex-row pt-4 transition-all duration-300 ${
                        isDropdownOpen
                    ...
                    >
                      ...
                      <div className="w-[45%] bg-[#FEE2E2] text-[black] mt-1 grid grid-cols-2 font-bold gap-3 p-6 overflow-y-auto">
              ...
              {isAdmin && (
                <Link
                  href={createCourseRoute.path}
                  className={`text-sm md:text-[13px] rounded-[3px] transition-all py-2 px-3 bg-[green] text-[white] font-bold hover:bg-[green]`}
                >
                  {createCourseRoute.label}
                </Link>
              )}
              ...
              {isAdmin && (
                <Link
                  ...
                  className={`text-sm md:text-[13px] rounded-[3px] transition-all py-2 px-3 bg-[#FDAB04] text-[black] font-bold hover:bg-[#FDAB04]`}
                >
                  {adminRoute.label}
                </Link>
              )}
              ...
              <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                ...
                >
                  <div className="flex flex-col overflow-scroll mt-4 gap-3">
                    ...
                    {/* Conditionally render admin login link in mobile menu */}
                    {isAdmin && (
                      <Link
                        href={adminRoute.path}
                        onClick={() => setIsSheetOpen(false)}
                        className={`p-3 rounded-lg font-bold bg-[#E79D09] text-white hover:bg-[#FDAB04]`}
                      >
                        {adminRoute.label}
                      </Link>
                    )}
