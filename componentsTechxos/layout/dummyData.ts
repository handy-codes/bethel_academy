export const dummyData = [
    {
      id: 1,
      img: "https://media.istockphoto.com/id/1500238408/photo/program-code-development-icon-on-a-digital-lcd-display-with-reflection.jpg?b=1&s=612x612&w=0&k=20&c=PB45SiRelu95ne_GCzPcNJ7XZ0eN1nB_c-nBIAB1dFg=",
      title:"FRONTEND DEVELOPMENT (FRD)",
      comment: "Create websites people will love, using sleek and user-friendly designs.",
      reading: "LEARN MORE",
      link: "/pages/frontend",
    },
    {
      id: 2,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQogbsuhTUL0o0dbW7dA-4-6ehszkJKygfzFA&s",
      title: "FULLSTACK DEVELOPMENT (FSD)",
      comment: "Take a step further, convert your website to a web app via a backend.",
      reading: "LEARN MORE",
      link: "/pages/fullstack",
    },
    {
      id: 3,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTCcXgXtSbEhekxJsYVg0FnWWjBsCrjO0HsyA&s",
      title: "DATA SCIENCE & ANALYTICS (DSC)",
      comment: "Leverage Statistics and Machine Learning to extract knowledge from data.",
      reading: "LEARN MORE",
      link: "/pages/data-science",
    },
    {
      id: 4,
      img: "https://media.istockphoto.com/id/1494104649/photo/ai-chatbot-artificial-intelligence-digital-concept.jpg?b=1&s=612x612&w=0&k=20&c=cUerJsSIULTLDjcXXP8asl1Wd9AOTvIcEI4l0IMeC9M=",
      title: "ARTIFICIAL INTELLIGENCE & MACHINE LEARNING (AIML)",
      comment: "Simulate human intelligence in machines, to reason, learn and solve problems.",
      reading: "LEARN MORE",
      link: "/pages/ai-ml",
    },
    {
      id: 5,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTfXeljsQQ9ezIVI7ekGLthBUBdxZ9ZmKkQFA&s",
      title: "SOFTWARE DEVELOPEMNT (SOD)",
      comment: "Translate requirements into functional software, prioritizing security, and efficiency.",
      reading: "LEARN MORE",
      link: "/pages/softwar-devt",
    },
    {
      id: 6,
      img: "data:image/jpeg;base64,",
      title: "DIGITAL MARKETING (DGM)",
      comment: "Connect your client with a target audience, promote brands through online channels.",
      reading: "LEARN MORE",
      link: "/pages/digital-marketing",
    },
    {
      id: 7,
      img: "https://i.ibb.co/xqN3xy1J/Gemini-Generated-Image-v5ipemv5ipemv5ip.jpg",
      title: "UI/UX DESIGN (UIX)",
      comment: "Craft the visual and interactive elements of a product. Create unbeatable brands.",
      reading: "LEARN MORE",
      link: "/pages/ui-ux",
    },
    {
      id: 8,
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQF-4cK7qw8-AroSfhE44AvDOvA-cyFM5p9ww&s",
      title: "CYBERSECURITY (CYB)",
      comment: "Protect systems, networks, and databases from digital attacks and unauthorized access.",
      reading: "LEARN MORE",
      link: "/pages/cybersecurity",
    },
    {
      id: 9,
      img: "https://i.ibb.co/4nDmr2nb/Gemini-Generated-Image-72ww6w72ww6w72ww.jpg",
      title: "Project Management (PMG)",
      comment: "Project management isn’t just Gantt charts—it’s leadership, risk-taming, and diplomacy. You’ll negotiate with stakeholders, crush bottlenecks, and turn “impossible deadlines” into “mission accomplished.”.",
      reading: "LEARN MORE",
      link: "/pages/project-mgt",
    },
    {
      id: 10,
      img: "https://i.ibb.co/1JpnGRVX/Gemini-Generated-Image-sbecuesbecuesbec.jpg",
      title: "Graphic Design (GRD)",
      comment: "From designing viral social media campaigns to crafting brand identities that outlive trends, graphic designers are the visual architects of culture.",
      reading: "LEARN MORE",
      link: "/pages/graphic-design",
    },
    // {
    //   id: 11,
    //   img: "https://media.istockphoto.com/id/636332456/photo/online-education-concept.jpg?b=1&s=612x612&w=0&k=20&c=B3Qb1fWVWdKcibGsE_ikxdpKjWkwKrIFtWzL7vJyq5c=",
    //   title: "Mathematics (JSS)",
    //   comment: "Meet Mathematics, the easiest but often poorly-thought subject. Turn it into your best subject. Crush Junior WAEC Maths with Confidence – Master Numbers Like a Pro!",
    //   reading: "LEARN MORE",
    //   link: "/pages/mathematics-jss",
    // },
]