...
export default function ChatWidget() {
  ...
  const lastMessageIndexOnOpenRef = useRef(0); // Track message count at chat open
  ...
  useEffect(() => {
    if (typeof Audio !== "undefined") {
      popupSoundRef.current = new Audio("/chat-popup.mp3");
      replySoundRef.current = new Audio("/chat-reply.mp3");
  ...
  // Play bot reply sound when new bot messages arrive
  useEffect(() => {
    if (isOpen && messages.length > lastMessageIndexOnOpenRef.current) {
      const lastMessage = messages[messages.length - 1];
      if (lastMessage.bot && replySoundRef.current) {
        replySoundRef.current.currentTime = 0;
        replySoundRef.current.play().catch(console.error);
      }
    }
  }, [messages, isOpen]);
  ...
  const toggleChat = () => {
    if (!isOpen) {
      // Play popup sound when opening chat
      if (popupSoundRef.current) {
        popupSoundRef.current.currentTime = 0;
        popupSoundRef.current.play().catch(console.error);
      }
      // Remember current message count
      lastMessageIndexOnOpenRef.current = messages.length;
    }
    setIsOpen(!isOpen);
  };
