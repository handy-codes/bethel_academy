...
export async function GET(req: Request) {
  try {
    ...
    if (!liveClass) {
      console.log("[LECTURE_GET] Frontend Development course not found, creating it");
      try {
        ...
        liveClass = await db.liveClass.create({
          data: {
            title: "Frontend Development",
            description: "Lead the Charge to Success with Frontend Development! Master project planning, execution, and delivery.",
            startTime: now,
            endTime: endDate,
        ...
        console.log("[LECTURE_GET] Created Frontend Development course with price:", liveClass.price);
      ...
      const now = new Date();
      const needsUpdate = liveClass.startTime < now || liveClass.price !== 150000;
      ...
      if (needsUpdate) {
        ...
        liveClass = await db.liveClass.update({
          ...
          data: {
            ...
            price: 50000 // Ensure price is 150,000 NGN
          },
          include: {
            lecturer: true
          }
