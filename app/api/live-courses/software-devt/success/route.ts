...
export async function POST(req: Request) {
  try {
    ...
    // Get dynamic payment data from frontend
    const { transactionId, status, flwRef, txRef, currency, amount } = await req.json();

    console.log("Payment success data:", {
      transactionId, status, flwRef, txRef, currency, amount, clerkUserId
    });
    ...
    // Calculate course end date (12 weeks from now)
