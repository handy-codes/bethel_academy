  ...
  try {
    ...
    try {
      ...
      if (!userEmail || !userEmail.includes("@")) {
        return new NextResponse("Valid email required for payment", {
          status: 400,
        });
      }
    } catch (clerkError) {
      console.error("Failed to fetch user from Clerk:", clerkError);
      return new NextResponse("Could not retrieve user information", {
        status: 500,
      });
    }
    ...
    if (!user) {
      ...
      if (existingUserByEmail) {
        ...
        try {
          ...
          return new NextResponse("Could not create user record", {
            status: 500,
          });
        }
      }
    ...
    // Find the Mathematics (JSS Module) Development course
    const liveClass = await db.liveClass.findFirst({
      where: {
        title: "Mathematics (JSS Module) Development",
        isActive: true,
        endTime: {
          gt: new Date(),
        },
    ...
    if (existingPurchase) {
      ...
      return NextResponse.json({
        ...
        redirect_url: `${process.env.NEXT_PUBLIC_APP_URL}/pages/mathematics-JSS/success`,
    ...
    console.log(
      "Raw price from database:",
      liveClass.price,
      typeof liveClass.price
    );

    // Get user's country code (from geolocation or test param)
    let countryCode = await getUserCountryCode();
    if (!countryCode) countryCode = "NG"; // fallback

    // Determine currency and amount for the country
    const pricing = countryPricing[countryCode] || defaultPricing;
    ...
    try {
      ...
      await db.liveClassPurchase.create({
        data: {
          ...
          liveClassId: liveClass.id,
          amount: price,
          currency, // <-- Ensure currency is set from detected country
          status: "PENDING",
          txRef: tx_ref,
          isActive: false,
          startDate: new Date(),
      ...
      if (createError.code === "P2002") {
        ...
        return new NextResponse("Could not create purchase record", {
          status: 500,
        });
      }
    }
    ...
    const redirectUrl = "/pages/mathematics-JSS/success";
    ...
    return NextResponse.json({
      ...
      currency, // <-- Return currency for client-side use if needed
      courseTitle: liveClass.title,
      studentId: user.id,
      studentEmail: userEmail,
      studentName: userName || "Student",
