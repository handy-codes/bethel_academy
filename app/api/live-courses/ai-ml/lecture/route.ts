...
export async function GET(req: Request) {
  try {
    ...
    if (!liveClass) {
      console.log("[LECTURE_GET] Artificial Intelligence course not found, creating it");
      try {
        ...
        liveClass = await db.liveClass.create({
          data: {
            title: "Artificial Intelligence",
            description: "Lead the Charge to Success with Artificial Intelligence! Master project planning, execution, and delivery.",
            startTime: now,
            endTime: endDate,
        ...
        console.log("[LECTURE_GET] Created Artificial Intelligence course with price:", liveClass.price);
      ...
      if (needsUpdate) {
        console.log("[LECTURE_GET] Updating course dates. Current price:", liveClass.price);
        const endDate = new Date();
        endDate.setDate(now.getDate() + 90); // 90 days from now
