...
import { LiveClassUserRole, PurchaseStatus } from "@prisma/client";
...
export async function GET(req: Request) {
  try {
    ...
    try {
      ...
      console.log("Retrieved user details from Clerk:", {
        userEmail,
        userName,
      });
    } catch (clerkError) {
    ...
    if (!user) {
      console.log(
        "[LECTURE_GET] User not found by Clerk ID, checking email:",
        userEmail
      );
      ...
      if (existingUserByEmail) {
        ...
        console.log(
          "[LECTURE_GET] Found existing user with matching email, updating with Clerk ID"
        );
        ...
        console.log(
          "[LECTURE_GET] Updated existing user with Clerk ID:",
          user.id
        );
      } else {
        ...
        console.log(
          "[LECTURE_GET] No database entry for this user, checking admin email status"
        );
        ...
        if (adminEmails.includes(userEmail.toLowerCase())) {
          console.log(
            "[LECTURE_GET] Creating admin record for known admin email:",
            userEmail
          );
          try {
            ...
            if (existingAdmin) {
              ...
              console.log(
                "[LECTURE_GET] Updated existing admin user:",
                user.id
              );
            } else {
            ...
            console.error(
              "[LECTURE_GET] Error creating/updating admin user:",
              userCreateError
            );
            return new NextResponse("Could not create/update user record", {
              status: 500,
            });
          }
        } else {
          ...
          console.log(
            "[LECTURE_GET] Not creating database entry for visitor:",
            userEmail
          );
    ...
    if (!liveClass) {
      console.log("[LECTURE_GET] Cybersecurity course not found, creating it");
      try {
        ...
        // Create the course with current dates
        liveClass = await db.liveClass.create({
          data: {
            title: "Cybersecurity",
            description:
              "Lead the Charge to Success with Cybersecurity! Master project planning, execution, and delivery.",
            startTime: now,
            endTime: endDate,
        ...
        console.log(
          "[LECTURE_GET] Created Cybersecurity course with price:",
          liveClass.price
        );
      } catch (courseCreateError) {
        console.error(
          "[LECTURE_GET] Error creating course:",
          courseCreateError
        );
        return new NextResponse("Could not create course record", {
          status: 500,
        });
      }
    } else {
      // Only update dates if needed
      const now = new Date();
      const needsUpdate = liveClass.startTime < now;

      if (needsUpdate) {
        console.log(
          "[LECTURE_GET] Updating course dates. Current price:",
          liveClass.price
        );
        const endDate = new Date();
        endDate.setDate(now.getDate() + 90); // 90 days from now
        ...
        console.log("[LECTURE_GET] Updated course dates");
      } else {
    ...
    if (user) {
      ...
      console.log("[LECTURE_GET] Purchase query:", purchaseQuery);
      console.log("[LECTURE_GET] Purchase found:", isPurchased);
      ...
      console.log(
        `User role: ${userRole}, isAdminOrHigher: ${isAdminOrHigher}, hasAccess: ${hasAccess}`
      );
    } else {
