...
export default function Page() {
  ...
  useEffect(() => {
    const checkDemoAccess = async () => {
      ...
      try {
        ...
        // Sync localStorage with backend state - store minimal data
        if (data.hasAccess) {
          localStorage.setItem("mathsDemoRegistered", "true");
          // Store only essential registration details
          if (data.registration) {
            localStorage.setItem("mathsDemoDetails", JSON.stringify({
              name: data.registration.name,
              class: data.registration.class,
              trainingDate: data.registration.trainingDate
            }));
          }
          
          // Check if this is a fresh registration (from localStorage)
          const isFreshRegistration = sessionStorage.getItem("freshRegistration") === "true";
          if (isFreshRegistration) {
            // Clear the flag
            sessionStorage.removeItem("freshRegistration");
            
            // Scroll to the Join Live Class button after a short delay
            setTimeout(() => {
              scrollToJoinButton();
            }, 100);
          }
        ...
        setHasDemoAccess(localStatus === "true");
        // Keep demo mode enabled if localStorage indicates registration
        setIsDemoMode(localStatus === "true");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Add a function to handle scrolling to the Join Live Class button
  const scrollToJoinButton = () => {
    // Try multiple selectors to find the Join Live Class button
    const selectors = [
      '#join-live-class-button', // Direct ID selector
      '#join-live-class-container', // Container ID
      '.join-live-class-button', // Class selector
      '.bg-blue-50', // Container of the Join Live Class button
      '.bg-green-700', // The button itself
      'button:contains("Join Live Class")' // Button with specific text
    ];
  ...
  // Add a new useEffect for handling fresh registration scroll
  useEffect(() => {
  ...
  // Add a new useEffect to check if we should scroll on page load
  useEffect(() => {
    // Check if user has registered for demo
    const hasRegistered = localStorage.getItem("mathsDemoRegistered") === "true";
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // Hide the form after successful submission and show success message
      setTimeout(() => {
  ...
  const renderLectureInfo = () => {
    if (!isSignedIn) {
      return (
        <div className="mt-6">
          <Link
            href={`/sign-in?redirect_url=${encodeURIComponent('pages/mathematics-jss')}`}
    ...
    // Check localStorage as a fallback if backend check failed
    const localStatus = localStorage.getItem("mathsDemoRegistered");
    const hasAccess = hasDemoAccess || localStatus === "true";
    ...
    // If user has demo access, show the Join Live Class button
    if (isDemoMode && hasAccess) {
      return (
        <div className="mt-6 p-4 bg-blue-50 rounded-lg shadow-sm" id="join-live-class-container">
          <div className="mt-4">
            <div className="mb-4">
              <p className="relative inline-block">
                Copy Meeting Passcode here: <span className="font-bold">i84j3h</span>
            ...
            <JoinLiveClassButton 
              ...
              className="join-live-class-button"
            />
          </div>
        </div>
      );
    ...
    // If not in demo mode and no lecture access, show payment
    return (
      <div className="mt-6">
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText("BS4ahN")
      .then(() => {
        setIsCopied(true);
      ...
      <section className="container mx-auto p-4 mt-4 flex flex-col md:flex-row gap-8">
        <div className="flex-1 text-black">
          ...
          <div className="font-semibold">
            ...
            {/* <div className="bg-[#00002E] text-[#D290F1] p-4 rounded-lg"> */}
            <div className="bg-[#3E50E2] text-[white] p-4 rounded-lg">
            ...
            {renderLectureInfo()}
            <p className="mb-4 my-4">Need Meeting Reminders?</p>
            <Link 
        ...
        {showSuccessMessage ? (
          <div
            id="contact"
            className="flex-1 text-black bg-green-50 p-6 rounded-lg shadow-md"
          >
